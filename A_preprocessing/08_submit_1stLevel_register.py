...
# Script to run
SCRIPT_PATH = '/user_data/csimmon2/git_repos/sym_pt/A_preprocessing/register_1stLevel.py'
...
for sub_num in subjects:
    ...
    for ses_num in sessions:
        ...
        # Wait for queue to clear before submitting next job
        while True:
            result = subprocess.run(['squeue', '-u', os.environ['USER'], '-h'], 
                                capture_output=True, text=True)
            running_jobs = len(result.stdout.strip().split('\n')) if result.stdout.strip() else 0
            
            if running_jobs < pause_crit:
                break
            
            print(f"🛑 Waiting... ({running_jobs}/{pause_crit} jobs running)")
            time.sleep(30)
