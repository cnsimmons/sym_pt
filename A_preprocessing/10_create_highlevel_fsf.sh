...
#
# Template from: long_pt/sub-004/ses-01 (3 runs)
# Replaces paths explicitly — no blanket sed on ses-01
#
...
while IFS=',' read -r sub ses rest; do
    ...
    # ── Explicit path replacements (no blanket sed) ──

    # Output directory
    sed -i "s|set fmri(outputdir) \".*\"|set fmri(outputdir) \"$session_dir/derivatives/fsl/loc/HighLevel\"|g" "$fsf_file"

    # Standard = first session's anat (NO .nii.gz extension for regstandard)
    sed -i "s|set fmri(regstandard) \".*\"|set fmri(regstandard) \"$dataDir/sub-${sub_clean}/ses-${first_ses}/anat/T1w_brain\"|g" "$fsf_file"
    ...
    # ── Remove extra entries from template (template had 3 runs) ──
