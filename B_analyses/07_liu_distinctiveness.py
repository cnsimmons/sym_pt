...
def compute_liu(beta_matrix, valid_cats, roi_category):
    if valid_cats is None or len(valid_cats) < 4 or roi_category not in valid_cats:
        return None, None
    ...
    liu_val = float(np.mean(fisher[pref_idx, nonpref]))
            ...
            for hemi in hemis:
                for category in CATEGORIES:
                    ...
                    base = {
                        ...
                        'cat_type': cat_type, 'roi_status': roi_status,
...
def main():
    ...
    for name, copes in cope_sets.items():
        ...
        print(f'  Saved: {OUTPUT_DIR}/liu_distinctiveness_{name}.csv')
        print(f'  Saved: {OUTPUT_DIR}/pairwise_correlations_{name}.csv')
