...
Contrast modes (--contrast):
  - 'others'   (default): copes 6/7/8/9 (cat-vs-all-others), subject thresh 0,
                          output dir tfce_votc_fdr.
  - 'baseline': copes 15/16/17/18 (cat-vs-baseline), subject thresh 2.58,
                          output dir tfce_votc_catbaseline.
...
OTHERS_DEFAULT_THRESH = 0.0
DEFAULT_CLUSTER_THRESH = 3.09  # ~p<.001 one-tailed

OUT_DIR = None

def get_out_dir(args):
    base = args.out_name if args.out_name else ('tfce_votc_catbaseline' if args.contrast == 'baseline' else 'tfce_votc_fdr')
...
def main():
    ...
    subject_threshold = args.thresh if args.thresh is not None else (BASELINE_DEFAULT_THRESH if args.contrast == 'baseline' else OTHERS_DEFAULT_THRESH)
