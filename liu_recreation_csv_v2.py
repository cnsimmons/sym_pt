...
Usage:
  python liu_recreation_csv.py                       # all subjects, full rewrite
  python liu_recreation_csv.py --sub 021             # single subject
  python liu_recreation_csv.py --sub 005 --append    # append/replace one subject
  python liu_recreation_csv.py --category evc        # rerun one category, merge
"""
import os
import sys
import time
...
def main():
    ...
    parser.add_argument('--sub', type=str,
                        help='Single subject (e.g., 021). Comma-separated for multiple: 005,039,082')
    parser.add_argument('--category', type=str,
                        help='Run single category (e.g., evc). Merges into existing CSV.')
    parser.add_argument('--append', action='store_true',
                        help='Append/replace rows for --sub in existing CSV (no full rewrite)')
    ...
    subs = load_subjects()
    if args.sub:
        # Support comma-separated list: --sub 005,039,082
        wanted = [f'sub-{s.replace("sub-", "").strip()}' for s in args.sub.split(',')]
        subs = {sid: subs[sid] for sid in wanted if sid in subs}
        missing = [s for s in wanted if s not in subs]
        if missing:
    ...
    print(f'Mode: {"APPEND" if args.append else "CATEGORY-MERGE" if args.category else "FULL REWRITE"}\n')
    ...
    for i, (sid, info) in enumerate(sorted(subs.items())):
        ...
        # Free memory between subjects (NIfTI cache grows otherwise)
        _CACHE.clear()
    ...
    if args.append and out.exists():
        ...
        new_subs = set(df_new['subject_id'].unique())
        n_before = len(df_old)
        df_old = df_old[~df_old['subject_id'].isin(new_subs)]
        df = pd.concat([df_old, df_new], ignore_index=True)
        n_dropped = n_before - len(df_old)
        if n_dropped:
            print(f'\nAppend: replaced {n_dropped} existing rows for {sorted(new_subs)}')
        print(f'Append: added {len(df_new)} rows for {len(new_subs)} subject(s)')
    elif args.category and out.exists():
        df_old = pd.read_csv(out)
        n_before = len(df_old)
    ...
    # Diagnostics (only for full rewrites — append/category outputs may be partial)
    if not args.append and not args.category:
        print('\nROI × hemi coverage:')
