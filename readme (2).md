...
`sym_pt_params.py` has **no `skip_codes` and no `should_skip`**, so scripts
importing it apply only `skip_subs`. `liu_recreation_csv_v2.py` compensates
with a local `EXTRA_SKIP` set. **Consolidating these two files into one is
outstanding work.** `params.py` is the superset.
...
| script | output | config | runtime |
|---|---|---|---|
| `01_univariate_analyses.py` | `D_liu/univariate_v1.csv` | `params.py` | ~20 min |
| `04_multivariate_analyses.py` | `D_liu/rsa_v1.csv` | `params.py` | ~60 min |
| `04b_multivariate_scramble.py` | `D_liu/rsa_v2_scramble.csv` | `params.py` | **[unverified]** |
| `liu_recreation_csv_v2.py` | `D_liu/liu_exact_replication_v2.csv` | `sym_pt_params.py` + its own `EXTRA_SKIP` | long |

**There are two extraction paths and they are easy to confuse.** The three
`verified/` scripts use `params.py` and feed the harmonization stage. The
fourth, `liu_recreation_csv_v2.py`, uses `sym_pt_params.py` plus a local
`EXTRA_SKIP` set, and feeds peak location via `calc_peak_coords_mni.py`. They
can drift out of sync: in Sept 2026 `liu_exact_replication_v2.csv` was five
months old with 69 subjects while `univariate_v1.csv` had 74, which made the
peak-location analysis run on a different cohort from every other measure
(34 controls against 36). **Rerun both paths together.**
...
## 4b · Peak location chain

Peak location does not read the harmonized CSVs. Its chain is:

```
liu_recreation_csv_v2.py  ->  D_liu/liu_exact_replication_v2.csv
calc_peak_coords_mni.py   ->  {processed_dir}/group_results/peak_coords/peak_coords_mni.csv
05_stats_harmony.py       ->  peak_distance rows
```

Both upstream files must be regenerated when the cohort changes, or
`peak_distance` will silently report a different n from every other measure.
...
| `D_liu/verified/03_wta_analyses.py` | preference mapping / WTA | `{processed_dir}/group_results/wta_percentages.csv` |
| `D_liu/wta_parcel_composition.py` | WTA within parcels | **[unverified]** |

`wta_percentages.csv` is **not** at the repo root, despite what older handoff
docs say. It is at `{processed_dir}/group_results/`. Like the peak-location
inputs, it does not regenerate with the main pipeline and was five weeks stale
in Sept 2026.
...
Outputs `D_liu/stats_results_harmonized_corrected.csv` and
`D_liu/tfce_clusters.csv`, or with a suffix if `--tag` is given.

Measures emitted: `peak_distance`, `peak_selectivity`, `selective_voxels`,
`sum_selectivity` (retired, kept for comparison), `distinctiveness`
(= preferred-category similarity), `geometry` (= between-category similarity),
`wta`, `wta_selective_prop`, `wta_within_cluster`. `peak_selectivity` and
`selective_voxels` were added in Sept 2026; before that the two measures the
Methods define had no group-comparison rows anywhere, and the figures notebook
carried hardcoded q-values instead.
