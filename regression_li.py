...
EXP_SUMSEL = {'object': -0.130, 'house': -0.124, 'face': -0.245, 'word': -0.389}     # tie at top
EXP_DIST   = {'word': 0.389, 'face': 0.245, 'object': 0.130, 'house': 0.124}     # tie at bottom (reverse)
...
def run(csv, valcol, measure, exp_dict, higher_is_better):
    ...
    ord_str = ' > '.join(k for k, _ in sorted(exp_dict.items(), key=lambda x: -x[1]))
    print(f"\n{'='*70}\n{measure}  col='{valcol}'  (higher_is_better={higher_is_better})"
          f"\n  predicted: {ord_str}\n{'='*70}")
    print(f"{'patient':>12} {'hemi':>5} {'beta':>7} {'p':>7}   effect(obj,hou,fac,wor)")
    rec = {'l': [], 'r': []}          # beta by intact hemi
    word_rh = []
    for sid in pts:
        ...
        _lr = linregress(exp_vec, eff); rho, pv = _lr.slope, _lr.pvalue
        rec[hemi].append(rho)
    ...
    def summarize(label, arr):
        ...
        line = f"  [{label}]  n={len(arr)}  mean beta={arr.mean():+.3f}  median={np.median(arr):+.3f}"
        if len(arr) >= 3:
            w = wilcoxon(arr)
...
run(UNIV, 'sum_selec_norm',      'SUM-SEL (extent)',       EXP_SUMSEL, higher_is_better=True)
run(RSA,  'liu_distinctiveness', 'DISTINCTIVENESS',        EXP_DIST,   higher_is_better=False)