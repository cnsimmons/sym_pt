#!/usr/bin/env python3
"""
rsa_nordt_primary.py — Nordt et al. (2023, Nat Commun 14:8010) distinctiveness.
...
WHAT IS THE SAME
----------------
Sphere construction, peak-finding machinery, and session handling are imported
unchanged from 04_multivariate_analyses.py. ONE ROI DEFINITION IS OVERRIDDEN:
word_VWFA now peaks on cope 9 (Word-all) instead of -cope 13. See WORD CONTRAST
FIX below.
...
Restricted to the 4 primary ROIs (face_FFA, word_VWFA, house_PPA_strict,
object_LOC) so it can run alongside the full version.

Output: D_liu/rsa_nordt_primary_cope9.csv
...
Usage:
  python rsa_nordt_primary.py
  python rsa_nordt_primary.py --n-runs 2
"""
import argparse
import glob
...
OUT = GIT / 'D_liu' / 'rsa_nordt_primary_cope9.csv'

# only the four primary ROIs
PRIMARY = ['face_FFA', 'word_VWFA', 'house_PPA_strict', 'object_LOC']
...
mv.ROIS = {k: v for k, v in mv.ROIS.items() if k in PRIMARY}

# ---------------------------------------------------------------------------
# WORD CONTRAST FIX
# cope 13 is LABELLED "Face-Word" but design.con row 13 is:
#     +1 at col1 (Face), -1 at col4 (House temporal derivative)
# i.e. Face - House_deriv. There is NO word regressor in it.
# word_VWFA was (13, True) = -cope13 = House_deriv - Face, so the word ROI was
# never centred on anything word-related.
# cope 9 ("Word-all") row 9 is a proper contrast:
#     -0.25 Face, -0.25 House, -0.25 Object, +1 Word, -0.25 Scramble
# Verified in sub-004/ses-06/run-01/1stLevel.feat/design.con.
mv.ROIS['word_VWFA'] = (9, False)
print(f"WORD FIX: word_VWFA peak contrast -> cope 9 (Word-all), was cope 13 negated")
# ---------------------------------------------------------------------------
